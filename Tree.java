...
public class Tree {
	...
	public boolean insertNode(Node node) {
		...
		if(root == null)
			root=node;
		else if(!(hashset.contains(node.getValue()))){
			...
			while(!inserted)
			{
				...
				{
					//before we move temp to its left, check if its left child is empty
					if(temp.getLeft() == null)
					{
						//if the left child is alrady empty, just assign node
						temp.setLeft(node);
						node.setParent(temp);
		...
		/*while(!q.isEmpty())
		{
			...
			System.out.println("All thw nodes are:");
	...
	public boolean deleteNode(Node node) {		
		...
		//right start
			...
			else if(node.getLeft()!=null && parent.getRight()==node) {
				System.out.println("dnsad");
		...
		//right end
			...
			System.out.println("getleft = null");
			 Node parent= node.getParent();
			if(root == node) {
				root = node.getRight();
			}
			else if(node.getRight()!=null && parent.getLeft()==node) {
			...
			System.out.println("inside elseloop");
			Node temp5 = node;
			Node parent1 = null;
			//if(temp5.getRight()!=null) {
				temp5 = temp5.getRight();
				while(temp5.getLeft()!=null) {
					temp5.setLeft_child_of_parent(true);
					temp5 = temp5.getLeft();
					System.out.println("inside while");
					
				}
				node.setValue(temp5.getValue());
				
				parent1 = temp5.getParent();
				System.out.println("parent" + parent1.getValue());
				//parent1.setValue(temp5.getLeft().getValue());
				
				if(temp5.isLeft_child_of_parent()){
					parent1.setLeft(temp5.getRight());
				
				}
				else {
					System.out.println("else loop");
					parent1.setRight(temp5.getRight());
				}
			//}
			...
			/*if(temp.getRight()!=null) {
				while(node.getValue() < temp.getRight().getValue()) {
					temp = node.getRight();
					
					node.setValue(node.getLeft().getValue());
					node.getRight().setValue(temp.getValue());
